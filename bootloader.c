...
void BTL_main(void)
{
    ...
    BTL_sendEcho(D_BTL);                                                // 发送bootloader版本号等信息
    while (1)
    {
        D_CLRWDT();
...
{
#if 0
    unsigned char xdata uartBuff[20];
    unsigned char idata rtl;
#endif
    while (1)
    {
...
void BTL_Updata(void)
{
    ...
    unsigned int  idata base = 0;
    unsigned char idata chk;
...
//  bootAddr[2] = *(unsigned char code *)(0x0002);      // 保存flash0x0000-0x0002三个字节的程序跳转信息
    ...
    BTL_sendEcho(D_START);
    BTL_memClr((unsigned char *)uartBuff, 20);                      // 清空数据缓冲
    //BTL_memClr((unsigned char *)BTL_tempBuff, D_SECTORSIZE);
    ptr = 0;
    while (1)
    {
        D_CLRWDT();
            ...
            if (ptr >= D_FRAMELEN )
            {
                if (uartBuff[D_FRAMESTOPLC] == '#')                   // 接收完整
                {
                    ...
                    if (chk == uartBuff[D_CHKLC])                           // hex校验正确
                    {
                        ...
                        switch (uartBuff[D_FLAGLC])
                        {
                            case D_UPDATE:                                      // 接收升级数据
                            {
                                base = D_UPDATALEN * uartBuff[D_LINELC];
                                
                                if (BTL_IAPWrite(currentSector, base, &uartBuff[D_DATESTARTLC]))
                                {
                                    BTL_sendEcho(D_NEXTFRAME);                  // 向上位机请求发送下一帧数据
                                }
                                else
                                {
